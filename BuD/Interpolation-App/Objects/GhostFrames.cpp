#include "GhostFrames.h"

GhostFrames::GhostFrames(BuD::Scene& scene)
	: m_FrameEntity(scene)
{
	...
	shaderPipeline.m_VertexShader = BuD::ShaderLoader::VSLoad("Resources/Shaders/ghost_frames_vs.hlsl", { 2 * sizeof(dxm::Matrix) });
	shaderPipeline.m_PixelShader = BuD::ShaderLoader::PSLoad("Resources/Shaders/ghost_frames_ps.hlsl", { 2 * sizeof(dxm::Matrix) });
	...
	instanceComponent.m_InstanceCallback = [this]()
	{
		...
		data.m_Data = reinterpret_cast<uint8_t*>(m_ModelMatrices.data());
		data.m_InstancesCount = m_ModelMatrices.size();
		data.m_Size = m_ModelMatrices.size() * sizeof(dxm::Matrix);
...
void GhostFrames::EnableRendering()
{
	auto& renderable = m_FrameEntity.GetComponent<BuD::IRenderable>();

	renderable.RenderingPasses[0].m_ShouldSkip = false;
}

void GhostFrames::DisableRendering()
{
	auto& renderable = m_FrameEntity.GetComponent<BuD::IRenderable>();
...
void GhostFrames::UpdateModelMatrices(const std::vector<dxm::Matrix>& modelMatrices)
{
