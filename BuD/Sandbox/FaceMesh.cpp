	...
	: m_Grow(0.0f)
{
	...
	auto material = meshSegments[0].Material;

	LoadTextures(meshSegments[0].SegmentName);
	...
	renderPass.VertexShader = BuD::ShaderLoader::VSLoad(L"../x64/Debug/depthMapAbsorption_vs.hlsl", meshSegments[0].Layout, { sizeof(DepthStageCb) });
	...
	finalRenderPass.VertexBuffer = meshSegments[0].VertexBuffer;
	finalRenderPass.IndexBuffer = meshSegments[0].IndexBuffer;

	finalRenderPass.VertexShader = BuD::ShaderLoader::VSLoad(L"../x64/Debug/objModel_vs.hlsl", meshSegments[0].Layout, { sizeof(DepthStageCb), 2 * sizeof(dxm::Matrix) });
